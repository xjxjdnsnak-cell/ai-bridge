...
- After an MCP server process restart, the original task was still recognized as `running`.
...
- MCP server process restart was exercised while the task was still running.
...
1. Update validation records and check documentation consistency.
2. Decide whether to treat v0.2.1 as release-complete.
3. Consider stream-json reconnect/backfill capability in a later version.
